...
| 3 Architecture Design | `docs/03-architecture` | **done** for capture (ADR-0001…0004). Platform era: **ADR-0005 (accepted)** adopts parent `SLL-PROP-DATA-001` (frozen) → `001A` (frozen) → 001B (Phase 5, implemented — PR #18) → 001C–001H |
...
| 7 Development | `src/`, `services/` | extension 0.7.0 (provenance hashes, raw sync); lens-api 0.6.2 (ingest, `/raw`, `/provenance`, `/mcp` 11 tools, ordered retention, RULE_V1 → `extract.*`, GEO_RULE_V1 → `geo.*`, `audit.events`, `/observations`, `/extract/stats`, `/geo/stats`, `/quality/stats`, review CSV round-trip) |
