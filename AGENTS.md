...
| 7 Development | `src/`, `services/` | extension 0.7.0 (provenance hashes, raw sync); lens-api 0.6.1 (ingest, `/raw`, `/provenance`, `/mcp`, ordered retention, RULE_V1 extraction → `extract.*`, GEO_RULE_V1 → `geo.*`, `/observations`, `/extract/stats`, `/geo/stats`) |
