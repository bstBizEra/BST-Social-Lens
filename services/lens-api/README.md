...
## Housekeeping — `app/housekeeping/` (SLL-DATA-HK-001, read-only v0.1)

`GET /housekeeping/status` (and MCP `housekeeping_status`): per-stage **watermarks** (capture, raw, ingest, extract, geo, resolve, snapshot, publish, retention — last success/failure, records in/pending, oldest pending, lag, method version, rows behind version), **reconciliation** ratios (R-EVID records↔raw, R-SIGHT records↔capture events, R-EXTR content hash↔current observation, R-GEO location claims↔resolution, R-RES observation↔decision, R-PROP property↔observations, R-SNAP property↔snapshot, R-RET nothing past retention), stage **health** (healthy ≥ 0.99 / degraded ≥ 0.9 / failing / disabled) and aggregate **findings** (closed vocabulary; `auto_action_allowed` marks what HK-I1 lets the Housekeeper do itself). Lifecycle states per property (CURRENT/AGING/STALE/HISTORICAL, 30/90/180 d) appear under the snapshot watermark when resolution is enabled. A loop (`LENS_HK_INTERVAL_MIN`, default 30, 0 = off; `POST /admin/housekeeping/run?dry_run=`) persists each run to `housekeeping.runs/watermarks/checks/findings` (findings upserted by identity, resolved when they disappear, never deleted); `GET /housekeeping/findings?type=&status=&severity=` reads them (MCP `housekeeping_findings`). `GET /lineage/{id}` (MCP `lineage`) walks payload hash → record → observation → location → decision → property → snapshot → dataset version from any identifier shape (64-hex, `platform:post_id`, `obs:n`, `loc:n`, `dec:n`, `MP-…`, `snap:n`), returning producing job/version/run and counts only. The §9 Actor runs after persistence (`LENS_HK_ACT`, default on; `LENS_HK_MAX_ACTIONS` 500): REEXTRACT, RECOMPUTE_SNAPSHOTS (flag on), PURGE_BY_POLICY, MARK_RAW_NEEDED (→ `GET /raw/needed`, which the extension polls on sync to re-send bodies it still holds), BACKFILL_EVENTS — one `audit.events` row (`HOUSEKEEP`) per action, then verify; anything else stays a finding for a human.
