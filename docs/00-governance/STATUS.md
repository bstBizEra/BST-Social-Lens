...
| Deployment | lens-api runs container-free on bizera-wsl as `bst-lens-api.service` (systemd, runbook §2a) against local PostgreSQL; running `f8e3816` (lens-api 0.6.4) — `extract.*`, `geo.*`, `audit.*` applied additively at startup; `schema_market.sql` **not applied** (`LENS_RESOLUTION_ENABLED` unset → `/market/*`, `/resolution/stats`, `/admin/quality/recompute` return 404, verified after the #35 restart at 2026-09-17T15:29Z); extraction loop active (15 min); gazetteer empty until a Lao Data Map import. PostGIS/pg_trgm not yet installed (001D G1; `scripts/install-postgis.sh` ready). 1 record (Phase 5 smoke) — no real capture data yet |
...
Merged into the base since the previous snapshot: #34, #35 (merge `f8e3816`).

| PR | Branch | Head | Disposition | Acceptance contract |
|---|---|---|---|---|
| #36 | `docs/status-snapshot-f8e3816` | this PR | approve | snapshot refresh only |
