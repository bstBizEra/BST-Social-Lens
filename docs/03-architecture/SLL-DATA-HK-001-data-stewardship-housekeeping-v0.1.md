...
- Implementation state: §6 watermarks, §7 reconciliation, §8 findings and §10 `housekeeping.*` DDL implemented (`services/lens-api/app/housekeeping/`; loop `LENS_HK_INTERVAL_MIN`, `GET /housekeeping/status`, `GET /housekeeping/findings`, `POST /admin/housekeeping/run`, MCP `housekeeping_status`/`housekeeping_findings`); §5 `GET /lineage/{id}` + MCP `lineage` implemented over existing keys; §9 Actor implemented (`app/housekeeping/actor.py`: REEXTRACT, RECOMPUTE_SNAPSHOTS, PURGE_BY_POLICY, MARK_RAW_NEEDED, BACKFILL_EVENTS — bounded by `LENS_HK_MAX_ACTIONS`, `LENS_HK_ACT=0` = observe only; one `audit.events` row per action, verify → RESOLVED); `GET /raw/needed` + extension 0.7.4 re-queue close the raw loop. Data Control Center implemented as a Console tab (`console/index.html`, served by lens-api at `/`); the Portal page follows in Phase 7. Lineage node payload: job/version/run, counts, states — never text or contacts.
...
Safe auto-actions (v0.1): `REEXTRACT` for `EXTRACTION_MISSING`/`EXTRACTION_STALE`; `RECOMPUTE_SNAPSHOTS` for `SNAPSHOT_STALE`; `PURGE_BY_POLICY` for `RETENTION_OVERDUE`; `MARK_RAW_NEEDED` for `RAW_MISSING` (server lists the hashes at `GET /raw/needed`; the extension re-queues bodies it still holds on its next sync); `BACKFILL_EVENTS` for `RECORD_WITHOUT_EVENT` (provenance rows from `records.captured_at`, marked `ingest_source = housekeeper:backfill`). Each action: one `audit.events` row (`system:housekeeper` / `system` / `HOUSEKEEP` / queue `housekeeping`) and one `housekeeping.actions` row; a failed action leaves the finding OPEN with the error recorded; verify re-runs the checks and resolves what no longer detects. Everything else is a finding.
