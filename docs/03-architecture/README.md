...
| `SLL-DATA-HK-001-data-stewardship-housekeeping-v0.1.md` | **HK-001 Data Stewardship & Housekeeping (draft)** — cross-cutting control plane over the 001 pipeline: authority envelope (HK-I1: may detect/reconcile/retry/recompute/quarantine/flag/purge-by-policy; must never change market facts or human decisions), lineage from existing run tables, per-stage watermarks, pairwise reconciliation R-EVID…R-RET with stage health, closed finding vocabulary + quarantine reasons, safe-action list, lifecycle states CURRENT/AGING/STALE/HISTORICAL, `housekeeping.*` DDL (draft), Data Control Center, decisions Q1–Q5. §6–§8 implemented read-only (`GET /housekeeping/status`, MCP `housekeeping_status`) |
